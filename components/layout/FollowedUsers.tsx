...
const FollowedUsers = (props: Props) => {
  ...
  const followFilter = superstream.contract.filters.Followed(null,currentUser?.profile?.owner);

  const getFollowedUsers = async () => {
    const followedUsernames = currentUser?.profile?.followers;
    followedUsernames.forEach(async (item)=>{
      const _data = await superstream.getProfileByUsername(item);
      setFollowedUsers(followedUsers => [...followedUsers,_data])
    })
  }
