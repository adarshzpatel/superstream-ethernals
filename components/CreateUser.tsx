...
const CreateUser = ({ isOpen, setIsOpen }: Props) => {
  ...
  const {createUser} = useSuperstreamContract();

  const handleCreateUser = async () => {
    if(username && about){
      try{
        const tx = await createUser(username,about);
        ;
      } catch (err){
        console.error(err);
      }
              ...
              {!pending && (
                <div>
                  ...
                  <div className="flex flex-col gap-1 mt-4">
                    <label htmlFor="stream_name" className="text-gray-300">
                      Username
                    </label>
                    <input
                    value={username}
                    ...
                    />
                    <label htmlFor="about" className="mt-2">About</label>
                    <textarea
                    value={about}
                    required
                  ...
                  <div className="mt-4 flex gap-4">
                    ...
                    >
                      <UserAddIcon className="h-5 w-5" /> Create Account
                    </button>
                    <button
                      onClick={closeModal}
                      className="bg-rose-500  hover:bg-rose-400 active:bg-rose-600"
                    >
